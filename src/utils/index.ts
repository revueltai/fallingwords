...
/**
 * Creates an array of select options from numbers or custom values.
 *
 * @param {object} params - The configuration object
 * @param {number} [params.start] - Start number for range (inclusive)
 * @param {number} [params.end] - End number for range (inclusive)
 * @param {any[]} [params.values] - Custom array of values to create options from
 * @param {(value: any) => string} [params.labelFormatter] - Optional function to format label
 * @returns {Array<{label: string, value: any}>} Array of select options
 *
 * @example
 * // Create options from number range
 * createSelectOptions({ start: 1, end: 3 })
 * // => [{label: '1', value: 1}, {label: '2', value: 2}, {label: '3', value: 3}]
 *
 * @example
 * // Create options from array
 * createSelectOptions({ values: ['en', 'es'] })
 * // => [{label: 'en', value: 'en'}, {label: 'es', value: 'es'}]
 *
 * @example
